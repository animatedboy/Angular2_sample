...
export class HttpProxy extends Http {
	...
	*/
	...
	private httpRequest(request:InterceptedRequest): Observable<Response> {
		...
		var ret = this.runBeforeInterceptors(request);


		return ret.flatMap<InterceptedRequest, InterceptedResponse>((value: InterceptedRequest, index: number) => {
		...
		.catch<InterceptedResponse>((err: any) => {
